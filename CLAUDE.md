...
4. **听力挑战（listen）**：TTS 读英文 → 4 张中文卡选 1，8 轮，答对 +1 分、答错 -1 心；错词进复习盒子
5. **每日挑战（daily）**：日期种子选词（同天同设备同词），完成记最快时间 + 连续天数，同样抽随机事件；**不扣心**（0 心走 endGame 会把未完成的挑战标记 done，属回归）
...
- 弹层 `helpVisible` 控制，z-index 230，粉白渐变头 + 六节内容（怎么玩/六模式/连击特效/随机事件/学习系统/安装离线）；点遮罩或 ✕ 关闭
